"""Build, promote, and verify the authored Heroic Character Forge style."""
...
STYLE = MODEL_ROOT / "heroic_style.json"
...
BASES = {
    "elf_bald_female": {
        ...
        "blend": "elf_bald_female/canonical/heroic/elf_bald_female_heroic_render.blend",
    },
    "tiefling_bald_female": {
        "base_id": "tiefling-female-01",
        "name": "Tiefling Female Base",
        "blend": "tiefling_bald_female/canonical/heroic/tiefling_bald_female_heroic_render.blend",
    },
    "dwarf_bald_male": {
        "base_id": "dwarf-male-01",
        "name": "Dwarf Male Base",
        "blend": "dwarf_bald_male/canonical/heroic/dwarf_bald_male_heroic_render.blend",
    },
    "human_bald_male": {
        "base_id": "human-muscular-male-01",
        "name": "Muscular Human Male Base",
        "blend": "human_bald_male/canonical/heroic/human_bald_male_heroic_render.blend",
    },
}
...
def _working_root(character_id: str, camera_height: str) -> Path:
    return MODEL_ROOT / character_id / "working" / "heroic" / "views" / camera_height


def _model_paths(character_id: str) -> dict[str, Path]:
    blend = MODEL_ROOT / str(BASES[character_id]["blend"])
    root = blend.parent
    return {
        "root": root,
        "blend": blend,
        "manifest": root / "heroic_model_manifest.json",
    }
...
def _safe_clear(path: Path, character_id: str) -> None:
    ...
    allowed = (MODEL_ROOT / character_id / "working" / "heroic").resolve()
...
def _style() -> dict[str, object]:
    ...
    if (
        ...
        raise RuntimeError(f"Unsupported Heroic style profile: {STYLE}")
    return data
...
def _verify_style_model(
    ...
    if not paths["blend"].is_file() or not paths["manifest"].is_file():
        raise FileNotFoundError(f"Missing {character_id} Heroic style model")
    data = json.loads(paths["manifest"].read_text(encoding="utf-8"))
    expected = {
        "kind": "heroic_authored_character_model",
    ...
    for key, value in expected.items():
        if data.get(key) != value:
            raise RuntimeError(f"{character_id} Heroic model manifest differs at {key}")
    if not 3.5 <= float(data["heads_tall"]) <= 7.0:
        raise RuntimeError(
            f"{character_id} Heroic proportion ratio is {data['heads_tall']:.2f} heads"
        )
    return data
    ...
    *,
    ...
    if final_manifest.exists() and not force:
        raise FileExistsError(
            f"{character_id}/{camera_height} Heroic output exists; use --force"
        )
    if force:
    ...
    _run(
        [
            ...
            str(TIMING),
            "--idle-action",
            str(style["actions"]["idle"]),
            "--walk-action",
            str(style["actions"]["walk"]),
            "--run-action",
            str(style["actions"]["run"]),
            "--expected-model-kind",
            "heroic_authored_character_model",
            "--manifest-kind",
            "paired_heroic_sprite_render",
        ]
    )
...
def _verify_view(character_id: str, camera_height: str, style: dict[str, object]) -> dict[str, object]:
    ...
    if not render_manifest.is_file() or not pixel_manifest.is_file():
        raise FileNotFoundError(f"Missing {character_id}/{camera_height} Heroic outputs")
    pixel_command = [
        sys.executable,
        str(ROOT / "tools" / "blender" / "pixelize_sprite_sheets.py"),
        "--manifest",
...
def _update_specs(records: dict[str, object]) -> None:
    ...
    styles[STYLE_ID] = {
        "name": "Heroic",
        "description": "Artist-authored heroic proportions built for high-readability JRPG sprite finishing.",
    }
    ...
    for cameras in manifest["characters"].values():
        for camera in cameras.values():
            ...
            if not palette.is_file() or _sha256(palette) != camera["palette"]["sha256"]:
                raise RuntimeError(f"Promoted Heroic palette differs: {palette}")
            for sequence in camera["sequences"].values():
                for file_key, hash_key in (
                    ...
                    if not path.is_file() or _sha256(path) != sequence[hash_key]:
                        raise RuntimeError(f"Promoted Heroic asset differs: {path}")
                for gif in sequence["gifs"].values():
                    path = ASSET_ROOT / gif["file"]
                    if not path.is_file() or _sha256(path) != gif["sha256"]:
                        raise RuntimeError(f"Promoted Heroic GIF differs: {path}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--build-only",
        action="store_true",
        help="Build and verify ignored working outputs without promoting them.",
    )
    parser.add_argument(
        "--promote-only",
        action="store_true",
        help="Verify and promote existing working outputs without rendering.",
    )
    args = parser.parse_args()
    if args.force and args.check:
        parser.error("--force and --check are mutually exclusive")
    if args.build_only and (args.check or args.promote_only):
        parser.error("--build-only cannot be combined with --check or --promote-only")
    if args.promote_only and args.force:
        parser.error("--promote-only cannot be combined with --force")
    style = _style()
    ...
    if not args.check and not args.promote_only:
        blender = _blender()
    ...
    if args.build_only:
        print(f"Built and verified {len(targets) * len(cameras)} Heroic working views")
        return 0
    if args.check:
        if args.target == "all" and args.camera_height == "all":
            ...
            if manifest.get("style_config_sha256") != _sha256(STYLE):
                raise RuntimeError("Heroic manifest style hash differs")
            _verify_promoted(manifest)
        print(f"Verified {len(targets) * len(cameras)} Heroic Character Forge views")
        return 0
    ...
    _verify_promoted(manifest)
    print(f"Built and promoted {len(targets) * len(cameras)} Heroic Character Forge views")
    return 0
