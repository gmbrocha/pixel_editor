...
Character Forge sprite style and camera height are independent canonical recipe/UI dimensions. `Standard Pixel` preserves the approved existing art. `JRPG Chibi` non-destructively shortens limbs and torso, enlarges heads, preserves foot grounding and approved action timing, and renders every approved base/motion at Near Top-Down, Three-Quarter, and Low. Chibi reduction uses one shared 20-color palette per character/camera package, binary alpha, deterministic centering and cleanup, and a one-pixel interior silhouette outline. Generated component families are fitted to all three chibi cameras from style-specific 32-region sheets; hand-authored hair, blindfold, and approved Standard overrides remain gated to their declared Standard coverage.
...
## Blockers
None. All four configured models, both sprite styles, three camera heights, three motions, and the 25 four-base generated component families are promoted. The family catalog contains 300 Standard Low sheets plus 900 chibi sheets. The canonical preprocessing pass is part of the live family generator, approved manual replacements are hash-linked Standard overrides, and the 300 Standard sheets remain byte-identical to the editable `component_cleanup_v2` mirror. Generated garments without overrides remain editable first-pass pixel assets rather than final hand-cleaned costume art.
...
Standard elevated Character Forge views do not have angle-specific component sheets. Generated chibi views do have exact semantic regions and fitted components at all three cameras. Hand-authored hair, facial details, blindfolds, and approved garment overrides need separate chibi artwork before they can be offered in that style.

The base pixel art remains a deterministic reduction of 3D renders and will benefit from deliberate face/silhouette cleanup. Generated garments follow each model's rig-derived anatomy and therefore read more like fitted base clothing than authored loose geometry. The target models have fitting-region sheets but not the elf's complete per-frame slot/hide-mask authoring packages. Loose authored silhouettes and chibi hair/facial accessories remain future art work.
...
## Next Actions
Review `JRPG Chibi` across all four bases, three animations, three camera heights, and four directions in Character Forge. Continue Standard manual edits from the 300 live mirrors indexed by `animation_images_models/component_cleanup_v2/index.csv`; every Idle sheet has 14 columns. Register later approved Standard edits in `animation_images_models/component_override_sources.json`. Author chibi hair, face accessories, or loose silhouettes against `assets/character-forge/style_regions/jrpg_chibi/` rather than resizing Standard art.
...
## Recent Activity
2026-08-29: Implemented and promoted the complete canonical JRPG Chibi pipeline. Added non-destructive Blender pose-space proportion styling and ground preservation, paired beauty/weight-semantic rendering, fixed-direction centering, shared 20-color pixel reduction, binary alpha, silhouette outlining, clipping/coverage QA, one-command build/check orchestration, and tracked runtime sheets, palettes, region maps, reviews, and 144 GIFs for four bases x three cameras x three animations x four directions. Added `Sprite style` to Character Forge recipe schema 4 and the UI, fitted all 25 generated component families across every chibi camera (900 new sheets), and hid incompatible hand-authored Standard-only parts. Full chibi, family, and cleanup-mirror deterministic checks pass; the complete suite reports 208 passed and three optional motion-transfer tests skipped.
